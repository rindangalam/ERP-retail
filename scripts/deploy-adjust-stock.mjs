...
const FUNCTION_NAME = "adjustStock";
...
async function deploy(functionId) {
  ...
  const tarPath = path.join(tmpDir, "adjust-stock.tar.gz");
  rmSync(tarPath, { force: true });

  const funcDir = path.join(process.cwd(), "functions", "adjustStock");
  execSync(
  ...
  const deployment = await functions.createDeployment({
    functionId,
    code: InputFile.fromPath(tarPath, "adjust-stock.tar.gz"),
    activate: true,
    entrypoint: ENTRYPOINT,
    commands: COMMANDS,
  });
...
await deploy(fn.$id);
console.log("SELESAI: adjustStock ter-deploy.");
